		...
		for (iter = movelist->next; iter != NULL; iter = iter->next)
		{
			...
			else
			{
				...
				else
				{
					...
					score += CountBit(GetPotentialMoves(move_w, move_b, ~(move_b | move_w))) << 1;
					iter->move.score = score;
				}
			}
		}
	...
	UINT32 color)
{
	...
	for (iter = movelist->next; iter != NULL; iter = iter->next)
	{
		...
		else
		{
			...
			n_moves_wh = CreateMoves(move_w, move_b, &move_cnt);
			// 相手の着手可能数
			iter->move.score -= (move_cnt + CountBit(n_moves_wh & 0x8100000000000081ULL)) * (1 << 4);
			// 自分の４隅における安定度
