...
INT32 SearchEmpty_1(UINT64 bk, UINT64 wh, INT32 pos, INT32 beta);
INT32 SearchEmptyWLD_1(UINT64 bk, UINT64 wh, INT32 pos, INT32 beta);

/* function empty 1 */
INT32(*GetEmpty1Score[])(UINT64, UINT64, INT32, INT32) = {
	SearchEmptyWLD_1,
	SearchEmpty_1
};
	...
	INT32 alpha, INT32 beta, INT32 passed)
{
	...
	if (rev)
	{
		best = -GetEmpty1Score[g_solveMethod](wh ^ rev, bk ^ (pos_bit | rev), x2, -alpha);
	...
	if (rev)
	{
		eval = -GetEmpty1Score[g_solveMethod](wh ^ rev, bk ^ (pos_bit | rev), x1, -alpha);
	...
	UINT32 parity, INT32 alpha, INT32 beta, UINT32 passed)
{
	...
	if (rev)
	{
		eval = -SearchEmpty_3(wh ^ rev, bk ^ (pos_bit | rev), blank ^ pos_bit,
			parity ^ board_parity_bit[x3], -beta, -alpha, 0);
		if (eval >= beta) return eval;
		else if (eval > best)
		{
			best = eval;
	...
	UINT32 parity, INT32 alpha, INT32 beta, UINT32 passed)
{
	...
	if (rev)
	{
		best = -SearchEmpty_4(wh ^ rev, bk ^ (pos_bit | rev),
			blank ^ pos_bit, empty, parity ^ board_parity_bit[x1], -beta, -alpha, 0);
		if (best >= beta) return best;
		if (best > alpha)
		{
			alpha = best;
	...
	if (rev)
	{
		eval = -SearchEmpty_4(wh ^ rev, bk ^ (pos_bit | rev),
			blank ^ pos_bit, empty, parity ^ board_parity_bit[x2], -beta, -alpha, 0);
		if (eval >= beta) return eval;
		else if (eval > best)
		{
			best = eval;
	...
	{
		eval = -SearchEmpty_4(wh ^ rev, bk ^ (pos_bit | rev),
			blank ^ pos_bit, empty, parity ^ board_parity_bit[x3], -beta, -alpha, 0);
		if (eval >= beta) return eval;
		else if (eval > best)
		{
			best = eval;
	...
	{
		eval = -SearchEmpty_4(wh ^ rev, bk ^ (pos_bit | rev),
			blank ^ pos_bit, empty, parity ^ board_parity_bit[x4], -beta, -alpha, 0);
		if (eval >= beta) return eval;
		else if (eval > best)
		{
			best = eval;
	...
	{
		eval = -SearchEmpty_4(wh ^ rev, bk ^ (pos_bit | rev),
			blank ^ pos_bit, empty, parity ^ board_parity_bit[x5], -beta, -alpha, 0);
		if (eval >= beta) return eval;
		if (eval > best) {
			best = eval;
			if (best > alpha)
	...
	if (best == -g_infscore)
	{
		...
		{
			best = -SearchEmpty_5(wh, bk, blank, empty, parity, -beta, -alpha, 1);
		}
	}
